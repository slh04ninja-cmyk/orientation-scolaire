...
# Afficher la matière et la classe
...
with info_col2:
    ...
    for subj, cols in subject_cols.items():
        st.markdown("- **" + subj + "** — " + str(len(cols)) + " devoir(s)")
with info_col3:
    st.markdown("- **Élèves :** " + str(len(df_raw)))
    st.markdown("- **Score moyen :** " + f"{scores.mean():.1f}" + " / 100")
